...
public class LoggingAspect {
     ...
     *  포인트컷(pointcut)은 여러 조인 포인트들에 어드바이스(advice)를 적용할 곳을 지정한 것이다.
     *  해당 조인포인트에서 어드바이스가 동작한다.
     *
     * 설명.
     *  <포인트컷 표현식>
     *  execution([수식어] 리턴 타입 [클래스이름].이름(파라미터))
     ...
     * 설명.
     *  ex)
     *   execution(public Integer com.ojgiraffers.section01.advice.*.*(*))
     *   => com.ohgiraffers.section.advice 패키지에 속해 있는 바로 다음 하위 클래스에 파라미터가 1개인 모든 메소드
     *      이자 접근제어자가 public이고 반환형이 Integer인 경우
...
//        System.out.println("After Advice 동작");
    ...
    /* 설명. returning에 쓰인 이름이 반환값이 넘어오는 매개변수명과 일치해야한다.(result) */
    ...
    /* 설명. throwing 속성값과 매개변수명이 일치해야한다.(exception) */
     ...
     *  AroundAdvice의 조인포인트 매개변수는 ProceedJoinPoint로 고정되어 있다.
     *  JoinPoint의 하위 인터페이스로 우너본 조인포인트의 진행을 제어 가능
     *  조인포인트를 진행하는 호출을 잊는 경우가 자주 발생하기 때문에 주의해야하며
     *  가능한 최소한의 요건을 충족하면서도 가장 기능이 약한 어드바이스를 쓰는게 바람직 -> 라소스를 많이 사용
    * */
    @Around("logPointcut()")
