...
public class OrderService {
     ...
     *    Service로 넘어올지 충분히 고민한 후 매개변수를 작성하고 개발
     *    현재의 경우 사용자가 고른 메뉴들 각각의 코드 번호와 고른 메뉴 갯수, 그리고 서버의 현재 시간이
     *    담긴 채로 넘어왔다는 생각을 가지고 개발하자
    * */

    /* 설명. 서비스 메소드 내부에서 모든 DML 관련 작업에 예외없이 잘 동작하면 commit, 예외가 발생하면 rollback이 적용 */
    /* 설명. 전파행위 옵션 (트랜잭션의 동시성 제어)
...
//        System.out.println("menuCode = " + menuCode);
        ...
        /* 설명. 2. 주문한 메뉴 별로 Menu엔티티에 담아서 조회해오기(부가적인 메뉴의 정보 추출) */
...
//        );
        ...
        /* 설명. tbl_order_menu에도 주문한 메뉴 갯수만큼 주문한 메뉴를 추가(insert)한다. */
        ...
        for (int i = 0; i < orderMenuSize; i++) {
            ...
            orderMenu.setOrderCode(order.getOrderCode());   // entitiy는 setter가 없었으니 추가하자
    ...
    /* 설명. 주문 건에 대한 총 합계 금액 계산 메소드(orderMenus: 사용자의 주문내용, menus: 조회된 메뉴 전체 내용) */
