 ...
 *   한 쪽 분기를 정하여 최대 깊이까지 탐색을 마친 후 다른 쪽 분기로 이동하여
 *   다시 탐색을 수행하는 알고리즘이다.
* */
public class Application1 {
    ...
    /* 설명. 문제에서 요구하는 입력값을 받아 감염된 컴퓨터의 갯수(int/Integer)을 반환하는 메소드 작성 */
    public static Integer solution(String input) throws IOException {
        BufferedReader br = toBufferedReader(input);
        node = Integer.valueOf(br.readLine());
        edge = Integer.valueOf(br.readLine());

        /* 설명. 노드와 간선에 대한 정복가 담길 map을 작성(node의 갯수에 기반한 int형 2차원 배열) */
        map = new int[node + 1][node + 1];           // node의 번호와 인덱스 위피를 일치시키기 위해서 크기를 1 증가(0번 인덱스는 사용X)

        /* 설명. 방문 배열 작성(방문 배열을 작성하는 이유는 방문했던 노드를 다시 stack에 담지 않거나 다시 재귀 함수를 호춣 하지 않기 위해) */
        ...
        for (int i = 0; i < edge; i++) {
            ...
            /* 설명. 무방향 그래프로 처리되기 위해 노드 번호(인덱스 번호)을 반대로도 적용한다. */
//            map[a][b] = 1;
//            map[b][a] = 1;
            map[a][b] = map[b][a] = 1;
        }
