...
//        ArrayList aList = new ArrayList();
        Vector aList = new Vector();                      // 벡터로 변경해도 오류 없음
...
//        System.out.println("첫번째 저장된 값: " + (Integer)aList.get(0));        // 컴파일 에러는 없고 런타입 에러가 생긴다.
        System.out.println("두번째 저장된 값: " + aList.get(1));
        System.out.println("aList에 담긴 데이터의 크기: " + aList.size());       // ArrayList에 추가한 값의 크기(lenth 아님)
...
//        for (int i = 2; i < newArr.length - 1; i++) {     // 기존의 값을 뒤로 옮길 때 앞의 인덱스부터 옮기면 안됨
        ...
        aList.set(1, 778);
        aList.set(0, 777);      // 기존에 있는 값의 타입과 상관 없이 변경 가능
        System.out.println(aList);

        /* 설명. remove() 메소드로 원하는 안덱스의 값을 삭제할 수 있다. */
        aList.remove(1);
