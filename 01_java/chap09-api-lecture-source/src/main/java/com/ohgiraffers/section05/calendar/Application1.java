...
public class Application1 {
    public static void main(String[] args) {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일 hh:mm:ss E요일");  //요일을 영어로 나타내고 싶다면?
        String todayFormat = sdf.format(today);     //Date형으로 변경해야 사용가능
...
//        java.sql.Date sqlDate2 = (java.sql.Date)today;                    // 실제로는 sql.Date형이었던 날짜형을 util.Date형인 today에 담겨 있었다면 이 방법도 가능
        System.out.println(sqlDate);

        /* 설명. java.sql.Date -> java.util.Date */
        java.util.Date utilDate = sqlDate;
        System.out.println(utilDate);                                       // auto-upcasting, 다형성 적용
    }
}
