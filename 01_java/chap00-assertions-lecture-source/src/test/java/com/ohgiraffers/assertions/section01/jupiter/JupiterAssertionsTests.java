...
public class JupiterAssertionsTests {
    ...
    /* 목차. 1. assertEquals */
    /* 필기. assertEquals(expected, actual) 메소드는 실제 값과 기대 값의 일치여부를 동일성으로 판단 */
    @Test
...
//        Assertions.assertEquals(expected, result, 1);           // 오차범위 허용
        Assertions.assertEquals(expected, result, "정확하게 작성하세요");           // 오류 메세지 띄움
    }

    /* 필기. assertNotEquals(expeced, actual) 메소드는 실제 값과 기대 값의 불일치 여부를 동일성으로 판단 */
    @Test
    @DisplayName("인스턴스 동일 비교 테스트")
    void testAssertNotEqualsWithInstanses() {
        ...
        /* 설명. equals()와 hashCode()가 오버라이딩 되어 있다면 동등비교로 판단 */
    }
    ...
    /* 목차. 6. assertAll */
    @Test
    @DisplayName("동시에 여러 가지 값에 대한 검wmd을 수행하는 경우 테스트")
    void testAssertAll() {
    ...
    void testAssertThrows() {
        ...
        Exception exception = Assertions.assertThrows(
                Exception.class,
                () -> validator.checkedDividableNumber(firstNum, secondNum)
        );
