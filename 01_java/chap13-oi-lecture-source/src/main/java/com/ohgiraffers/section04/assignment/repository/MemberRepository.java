...
public class MemberRepository {
    ...
    public MemberRepository() {
        ...
        /* 설명. 회원 가입 기능 추가 후 이제는 파일이 기존에 존재하면(처음이 아니므로) 회원 3명으로 초기화 하지 않는다.*/
...
//        System.out.println("========= repository에서 회원 정보 다 읽어왔는지 확인 */=========");
//        for(Member m: memberList) {
//            System.out.println(m);
//        }
    }

    /* 설명. 회원이 담긴 ArrayList를 넘기면 파일에 출력하는 기능 */
...
//        Member lastestMember = memberList.get(memberList.size() - 1);       // 가장 최근에 가입한 회원
//        return lastestMember.getMemNo();                                    // 그 회원의 번호
        return memberList.get(memberList.size() -1)
                         .getMemNo();
    }

    /* 설명. 기존 회원(객체)에 이어서 파일 출력을 하고 추가한 객체의 수를 반환(feat. DML 작업의 결과는 int) */
    /* 설명. 객체 출력을 할 예정인데 기존 ObjectOutputStream 대신 새로 정의한 스트림으로 회원 한명 출력하기(feat. 이어쓰기) */
    ...
    public int deleteMember(int memNo) {
        for (int i = 0; i < memberList.size(); i++) {
            if(memberList.get(i).getMemNo() == memNo){      // 지우려는 회원과 같은 회원 번호인 index 찾기
                ...
                /* 설명. 프로그램 사엥서 회원을 관리하는 memberList 뿐 아니라 DB(회원 파일)도 같이 적용되게 함   */
                memberList.remove(i);
