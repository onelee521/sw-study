...
public class Car {
    ...
    private boolean isOn = false;    // false 생략가능
    ...
    public void startUp() {
        if(isOn) {                  // 시동이 걸려있으면
            ...
            this.isOn = true;       // isOn 앞에는 this.이 있는거지만 this.을 생략해도 자동으로 적힌다.
            System.out.println("시동을 걸었습니다.");
        }
    }

    /* 설명. 시동이 걸린 상태인 경우에만 호출 시마다 10km/h씩 속도를 증가시킨다. */
    public void go() {
    ...
    /* 설명. 시도잉 걸려있는 상태인 경우 시동을 끄고, 이미 꺼진 상태라면 끌 수 있고 주행 중이라면 끌 수 없다. */
    public void turnOff() {
        if(isOn){
            if(speed > 0){
