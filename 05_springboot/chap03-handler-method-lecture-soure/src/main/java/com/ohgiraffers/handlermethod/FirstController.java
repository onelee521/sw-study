...
//    public String regist() { //오류남
//        return "/first/regist";
//    }
    public void regist() {}

    /* 설명. request를 쓰게 된다면 request의 개념은 사용자의 입력값이 존재, model은 동적 페이지의 재료를 담는 용도로 쓰자. */
    @PostMapping("regist")
//    public String registMenu(HttpServletRequest request) {
    ...
    /* 설명. */
    /* 설명.
     *  request의 parameter로 넘어오는 값들의 key 값과 일치하는 변수명을 작성하고
     *  @RequestParam 어노테이션을 적용하면 알아서 값을 꺼내고 해당 매개변수의 자료형에 맞게 변환까지 해준다.
     *  @RequestParam 어노테이션은 생략도 가능하다.
     *  (스프링부터는 컨트롤러의 파싱 작업이 간소화 된다는 것을 알 수 있다.)
    * */

    /* 설명.
     *  @RequestParam의 속성
     *  1. defaultValue: 사용자의 입력 값이 없거나("") 아니면 request의 parameter 키 값이 일치하지 않는
     *                   매개변수 명 사용 시 매개변수가 가질 기본 default 값을 작성한다.
     *  2. name: request parameter의 키 값과 다른 매개변수 명을 사용하고 싶을 때 request parameter의
     *           키 값을 작성한다.
     *  속성 사용 시에는 어노테이션 생략이 불가능하다.
    * */
    @PostMapping("modify")
    public String modifyMenu(Model model, @RequestParam(defaultValue = "디폴트", name = "name") String modifyName,        // 추가 속성이 없을 때 @RequestParam 생략 가능
                                          @RequestParam(defaultValue = "0") int modifyPrice) {  // 값을 생략하면 nullPointException 발생, 값이 없는데 형변환을 할 수 없음, 디폴트 추가(어노테이션 생략불가)
    ...
    /* 설명.
     *  Map으로 받으면 위에 메소드와 매개변수가 달라 오버라이딩 된다.(메소드명이 같음)
     *  Map은 자동으로 자료형 변환이 되지 않아 직접 자료형 변환을 해주어야 한다.
    * */
    @PostMapping("modify2")
    ...
    /* 설명. @ModelAttribute 어노테이션을 활용하면 커맨드 객체를 모델에도 담아주며 어트리뷰트의 키 값을 지정할 수 있다.(키 값이 없으면 타입의 낙타봉 표기법이 키 값이다.) */
    // DTO의 getter와 setter은 프레임워크 내부에서 자동으로 사용하기 때문에 만들어주어야 함
    @PostMapping("search")
