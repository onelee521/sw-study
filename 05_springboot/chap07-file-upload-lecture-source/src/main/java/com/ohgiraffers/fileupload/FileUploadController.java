...
//        System.out.println("빌드된 single 디렉토리의 절대경로: " + resource.getFile().getAbsolutePath());
        ...
        String saveNamed = UUID.randomUUID().toString().replace("-", "") + ext;
        System.out.println("saveNamed = " + saveNamed);

        /* 설명. 우리가 지정한 경로로 파일을 저장하고,  */
        try {
            singleFile.transferTo(new File(filePath + "/" + saveNamed));

            /* 설명. db를 다녀오는 비즈니스 로직 구문 작성(db에 저장) */
            /* 설명. 비즈니스 로직이 성공하명 화면의 재료를 RedirectAttributes로 담는다.(flashAttribute) */
            rttr.addFlashAttribute("message", "파일 업로드 성공");
            rttr.addFlashAttribute("img", "uploadFiles/img/single/" + saveNamed);
            ...
            new File(filePath + "/" + saveNamed).delete();
        }
...
//        System.out.println("multiFileDescription = " + multiFileDescription);
        ...
        try {
            for (int i = 0; i < multiFiles.size(); i++) {
                ...
                /* 설명. DB에 저장할 사용자가 올린 하나의 파일 정보(DB테이블 컬럼 참고) */
            ...
            /* 설명. List<Map<String, String>> file로 쌓인 업로드 된 파일들을 찾아 일일이 다시 지운다. */
