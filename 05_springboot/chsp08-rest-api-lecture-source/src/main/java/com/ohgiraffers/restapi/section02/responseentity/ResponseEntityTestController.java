...
//        // ResponseMessage responseMessage = new ResponseMessage(200, "조회성공", responseMap);
//
//        return new ResponseEntity<>( responseMap, headers, HttpStatus.CREATED);
//    }

    /* 설명. 2. 빌더를 활용한 메소드 체이닝 방식으로  ResponseEntity 객체 만들기(요즘 유행) */
    /* 설명. 위에 예제보다 보기가 좋음. 매개변수가 추가되어도 아래로 쌓임 */
    ...
    /* 설명.
     *  기본에 우리가 배웠던 @RequestParam과 달리 json 문자열이 핸들러 메소드로 넘어올 떼는 @RequestBody를 붙이고
     *  받아내야 한다. json 문자열의 각 프로퍼티 별로 받을 수도 있지만 한 번에 하나의 타입으로 다 받아낼 때는 커맨드 캑체
     *  (UserDTO)를 활용해야 하며 커맨드 객체는 json 문자열의 프로퍼티 명과 일치해야 한다.
     *
     * 설명.
     *  {
     *    "id" : "user01"
     *  }
     *  이때 "id" : "user01"을 프로퍼티, "id"를 '프로퍼티명'이라고 한다.
    * */
    @PostMapping("/users")
    ...
    public ResponseEntity<?> modifyUser(@RequestBody UserDTO modifyInfo, @PathVariable int userNo) {
        ...
        /* 설명. 사용자가 넘겨준 수정하고자 하는 데이터로 회원정보를 수정 */
