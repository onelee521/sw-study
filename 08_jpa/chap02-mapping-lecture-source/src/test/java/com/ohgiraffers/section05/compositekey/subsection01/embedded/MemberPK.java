...
/* 설명.
 *  Embeddable 타입은 하나의 값의 묶음이자 불변객체로 다루는 타입
 *  (불변객체: setter가 없고 필드값에 변화를 주고 싶으면 새로운 필드를 가지는
 *   새로운 객체가 생성되어야 한다.)
 *   그리고 equal, hashCode를 반드시 오버라이딩 해야한다,
 *   VO와 유사하면 @EmbeddedId로 복합키를 표현하고자 할 때 쓰이기도 한다.
* */
@Embeddable
