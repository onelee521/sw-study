...
public class A_EntityLifeCycleTests {
    ...
    /* 필기.
     *  영속성 컨텍스트는 엔티티 매니저가 엔티티 객체를 저장하는 공간으로 엔티티 객체를 보관하고 관리
...
//        System.out.println("foundMenu = " + foundMenu);
        ...
        /* 설명. 영속 상태의 객체에서 값을 추출헤 담더라도 새로 생성되어 영속성 컨텍스트와 관련 없는 객체는 비영속 상태이다. */
    ...
    public void 영속성_객체_추가_테스트() {
        ...
        /* 설명.
         *  이 예제에서는 menuCode 필드 값에 직접 값을 주고자 한다.(auto_increment 개념 안쓸예정)
         *  @GeneratedValue 부분을 주석하고 테스트 작성
        * */
    ...
    @Test
    public void 준영속_clear_close_테스트() {
        Menu foundMenu1 = entityManager.find(Menu.class, 11);
        Menu foundMenu2 = entityManager.find(Menu.class, 12);
...
//        entityManager.clear();
        ...
        /* 설명. DB에서 새로 조회해온 객체를 영속 상태로 두기 때문에 전혀 다른 결과가 나온다. */
        assertEquals(5000, entityManager.find(Menu.class, 11).getMenuPrice());
        assertEquals(5000, entityManager.find(Menu.class, 12).getMenuPrice());
    }
    ...
    public void 병합_merge_삽입_테스트2() {
        ...
        assertNotEquals(menuToDetch.getMenuCode(), newMenu.getMenuCode());
    }
}
