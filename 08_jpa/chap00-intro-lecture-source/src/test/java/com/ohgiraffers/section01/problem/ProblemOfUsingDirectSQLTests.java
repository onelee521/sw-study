...
public class ProblemOfUsingDirectSQLTests {
    ...
    void closeConnection() throws SQLException {
        con.rollback();     // 테스트만하고 디비에 적용x
        con.close();
    }

    /*  */
    /* 설명. JDBC API를 이용해 직접 spl을 다룰 때 발생할 수 있는 문제점
     *  1. 데이터 변환, spl 작성, jdbc api 코드 등의 중복 작성(개발시간 증가, 유지보수성 저하)
     *  2. sql에 의존하여 개발
     *  3. 패러다임 불일치(상속, 연관관계, 객체 그래프 탐색, 방향성)
     *  4. 동일성 보장 문제
     * */

    /* 설명. 1. 데이터 변환, spl 작성, jdbc api 코드 등의 중복 작성(개발시간 증가, 유지보수성 저하) */
    ...
    void testDirectInsertSQL() throws SQLException {
        ...
        String query = "INSERT INTO TBL_MENU(MENU_NAME, MENU_PRICE, CATEGORY_CODE, " +
                "ORDERABLE_STATUS) VALUE (?,?,?,?)";

        //when
        PreparedStatement psmt = con.prepareStatement(query);
        psmt.setString(1, menu.getMenuName());
        psmt.setInt(2, menu.getMenuPrice());
        psmt.setInt(3, menu.getCategoryCode());
        psmt.setString(4, menu.getOrderableStatus());
    ...
    void testChangeSelectColumns() throws SQLException {
        ...
        while ((rset.next())) {
            ...
            menu.setMenuCode(rset.getInt("MENU_CODE"));
            menu.setMenuName(rset.getString("MENU_NAME"));
//            menu.setMenuPrice(rset.getInt("MENU_PRICE")); // 지운다면 쿼리도 같이 변경 시킴
    ...
    /* 설명. 3-1. 상속 문제
     *  객체 지향 언어의 상속 개념과 유사한 것이 데이터베이스의 서브타입엔티티이다. (서브타입의 별도 클래스로 나뉘었을 때)
    ...
    /* 설명. 3-2. 연관관계문제, 객체 그래프 탐색 문제, 방향성 문제
     *  객체지향에서 말하는 가지고 있는 (ASSOCIATION 연관관계 혹은 COLLECTION 연관관계) 경우 데이터베이스 저장구조와 다른형태이다.
     *
     * 설명.
     ...
     *   private String menuName;
     *   private int menuPrice;
     *   private Category category; // 이렇게 코드를 짤 수 있음 항상 조인이 일어날 때(중간객체 등), VO라고 함
     *   private String orderbleStatus;
     *  }
     * */
