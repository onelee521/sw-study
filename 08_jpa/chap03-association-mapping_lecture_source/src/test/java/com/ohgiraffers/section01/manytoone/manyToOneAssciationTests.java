...
public class manyToOneAssciationTests {
    ...
    /* 설명.
     *  Association Mapping은 Entity 클래스 간의 관계를 매핑하는 것을 의미(joi을 위함)
     *  이를 통해 객체를 이용해 데이터베이스의 테이블 간의 관계를 활용해 한 번에 객체로 조회할 수 있다.
     *
     * 설명.
     *  1. 다중성에 의한 분류
     *     : 연관 관계가 있는 객체관계에서 실제로 연관을 가지는 객체의 수에 따라 분류
     *      - N:1(ManyToOne) 연관관계
     *      - 1:N(OneToMany) 연관관계
     *      - 1:1(OneToOne) 연관관계
     *      - N:N(ManyToMany) 연관관계
     ...
     *     객체 간의 연관관계를 양방향으로 만들고 싶을 경우 반대 쪽에서도 필드를 추가해서 참조를 보관하면 된다.
     *     하지만 엄밀하게는 양방향 관계가 아니라 단방향 관계로 2개로 볼수 있다.
    ...
    * */
    ...
    public void 다대일_연관관계_객체_그래프_탐색을_이용한_조회_테스트(){
        ...
        Category menuCategory = foundMenu.getCategory();  // 객체 그래프 탐색
