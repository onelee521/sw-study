...
import com.ohgiraffers.springdatajpa.menu.sevice.MenuService;
...
@Controller
@RequestMapping("/menu")
@Slf4j          // 롬복으로 바로 로그 찍어줌
public class MenuController {
    private final MenuService menuService;

    /* 설명. 로그 생성해 보기 */
//    Logger logger = LoggerFactory.getLogger(MenuController.class);
    Logger logger = LoggerFactory.getLogger(getClass());  // 돌고있는 클래스 로그 찍어줌
...
//    }
    ...
    public String findMenuList(@PageableDefault Pageable pageable, Model model) {
        ...
        /* 설명. 화면에서 버츤을 그리기 위해 필요한 재료 준비(클래스(모듈화) 두개 추가) */
        return "menu/list";
    }
}
