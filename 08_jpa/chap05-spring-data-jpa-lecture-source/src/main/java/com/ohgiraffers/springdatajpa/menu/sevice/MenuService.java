package com.ohgiraffers.springdatajpa.menu.sevice;

import com.ohgiraffers.springdatajpa.menu.dto.MenuDTO;
import com.ohgiraffers.springdatajpa.menu.entitiy.Menu;
...
public class MenuService {
    ...
    public List<MenuDTO> findMenuList() {
        List<Menu> menuList = menuRepository.findAll(Sort.by("menuCode").descending()); // findAll 정렬 가능

        return menuList.stream().map(menu -> mapper.map(menu, MenuDTO.class)).collect(Collectors.toList());
    }

    /* 설명. 3. findAll(페이징 처리 후), 오버로딩이라 위에랑 메소드이름 같아도 됨 */
//    public Page<MenuDTO> findMenuList(Pageable pageable) {
//
//        /* 설명.
//         *  1. 넘어온 pageable에 담긴 페이지 번호를 인덱스 개념으로 바꿔서 인지 시킴
//         *  2. 한 페이지에 뿌려질 데이터의 크기
//         *  3. 정렬 기준
//        * */
