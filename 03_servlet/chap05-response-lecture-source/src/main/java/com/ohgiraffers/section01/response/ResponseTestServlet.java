...
//        super.doGet(req, resp);  // super는 뭐하는거지

        /* 설명.
         *  1. 요청 받기(http method GET/POST 요청에 따른 parameter로 전달 받은 데이터를 꺼낼 수 있다.)
         *  2. 비즈니스 로직 처리(db접속과 CRUD에 대한 로직 처리 시작 부분 -> 서비스 시작 계층 호출(MVC2구조 기반))
         *  3. 응답하기(문자열을 통한 동적인 웹(HTML) 페이지를 만들어서 스트림을 통해 내보내기)(SSR 기반)
        * */

        StringBuilder responseBluilder = new StringBuilder();
        responseBluilder.append("<!DOCTYPE html>\n")
                .append("<html>\n")
                .append("<head>\n")
                .append("</head>\n")
                .append("<body>\n")
