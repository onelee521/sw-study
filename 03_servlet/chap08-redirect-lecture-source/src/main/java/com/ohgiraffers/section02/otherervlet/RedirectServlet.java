package com.ohgiraffers.section02.otherervlet;
...
public class RedirectServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        System.out.println("redirect 이후 request의 attribute: " + req.getAttribute("text"));      //null

        /* 설명. 파라미터가 넘어오면 request 객체가 달라져도 값이 전달되어 온다. */
        System.out.println("redirect 이후 request의 parameter: " + req.getParameter("test"));

        /* 설명.
         *  redirect 시에는 request의 attribute 개념으로 다른 서블릿에게 값(상태)를 전달할 수 없다.
         *  왜냐하면 다음 서블릿으로 클라이언트가 새로 요청하면 기존의 request와 response 객체가 아닌
         *  새로운 request와 response 객체가 만들어지며 요청하기 때문이다.
         *
         * 설명.
         *  추가로, cookies나 session같은 공간을 활용하는 방법을 사용해서 값을 공유할 수도 있다.
        * */

        /* 설명.
         *  포워딩 vs 리다이렉팅
         *   - request나 response 객체를 유지하여 요청처리를 하고 싶다면 포워딩 사용
